...
@ActiveProfiles("test")
class PythonGeneratorDirectPacsIntegrationTest {

    private static final String GENERATED_FILE_PROPERTY = "pygen.acceptance.file";
    ...
    @Test
    void shouldIngestPythonGeneratedSampleAndExposeItThroughQidoWadoAndViewerApis() throws Exception {
        Path generatedPath = requireGeneratedFile();
        byte[] payload = PublicSampleLibrarySupport.readBytes(generatedPath);
        Attributes sourceDataset = PublicSampleLibrarySupport.readDataset(generatedPath);
        ...
        mockMvc.perform(get("/wado-rs/studies/{study}/series/{series}/instances/{instance}", studyUid, seriesUid, sopUid))
                ...
                .andExpect(result -> {
                    ...
                    assertThat(storedDataset.getString(Tag.PatientID)).isEqualTo(patientId);
    ...
    private Path requireGeneratedFile() {
        String configuredPath = System.getProperty(GENERATED_FILE_PROPERTY, "").trim();
        Assumptions.assumeTrue(!configuredPath.isBlank(), () -> "Missing -D" + GENERATED_FILE_PROPERTY + "=<generated-file>");
        Path generatedPath = Path.of(configuredPath).toAbsolutePath().normalize();
        Assumptions.assumeTrue(Files.exists(generatedPath), () -> "Generated file does not exist: " + generatedPath);
